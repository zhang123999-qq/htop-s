...
| 键 | 模式 | 排序依据 | 数据源 |
|---|---|---|---|
| `c` | CPU（默认） | CPU% 降序 | `/proc/[pid]/stat` 差分 |
| `m` | 内存 | MEM% 降序 | `stat` 第 24 字段 × 页大小 |
| `n` | 网络 | 持有的 socket 数降序 | 遍历 `/proc/[pid]/fd/*` 统计 `socket:[N]` |
| `i` | IO | `read_bytes + write_bytes` 速率降序（需 root） | `/proc/[pid]/io` 两次采样差分 |

**采集时机（重要设计取舍）**：`c`/`m` 复用每轮中帧已有的进程快照，零额外开销；
而 `n` 需要遍历全系统 fd、`i` 需要读全部 `/proc/[pid]/io`，
单次开销比读一个文件高两三个数量级。

因此 **`n` 与 `i` 的数据只在按下对应按键时采集**，之后由中帧周期性刷新，
不在每帧执行。这与 `p`（端口→进程映射）保持一致——
监控工具不能因为一个排序视图就把服务器拖慢。

`n`/`i` 模式下渲染层会多出一列指标（表头分别为 `CONN` / `IO`），`i` 的速率在 awk 内
完成单位换算，避免渲染层逐行 fork。

**已知局限**：
- `n` 统计的是进程持有的 socket 总数（含监听套接字），非严格的 TCP 连接数
- 非 root 用户只能看到自己的进程 fd，其他进程的连接数会偏低
- `i` 在非 root 下读不到其他用户的 `/proc/[pid]/io`，同样偏低
...
版本号在本项目有 **5 种不同语义**，混为一谈会改错 —— 尤其是把历史记录里的旧版本号
「顺手统一」，等于伪造测试结论：

| 层 | 语义 | 位置 | 发版时是否需改 |
|---|---|---|---|
| 1 | **程序版本** | `htop-s:5` 文件头注释<br>`htop-s:20` `VERSION=` 变量<br>`.build/p1` 同两处（构建源）<br>`dist/htop-s`（由构建生成） | **必须改**，且四处须一致 |
...
```bash
bash tests/check-versions.sh
```

以 `htop-s` 的 `VERSION` 为权威来源，逐层核对上述 5 类位置，并做两项反向检查：
分片拼接结果必须与 `htop-s` 逐字节一致；历史记录类版本号必须**仍然存在**（防止被误改）。
退出码 `0` = 一致，非 `0` = 有 `n` 处不一致。发版前后各跑一次。

#### 发版时必须同步的位置

```
htop-s:5              # 版本: X.Y.Z          ← 极易遗漏
htop-s:20             VERSION="X.Y.Z"
.build/p1:5,20        (同 htop-s, 改完需重新拼接)
README.md             安装示例 -v X.Y.Z
htop-s-使用手册.md     安装示例 -v X.Y.Z
install.sh            用法示例与 die 提示中的 -v X.Y.Z
```

改完执行 `bash tests/check-versions.sh` 确认全绿，再跑 `--selftest` 与集成测试。
